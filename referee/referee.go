// referee.go — Referee: event-driven scoring engine.
//
// Subscribes to EventLog.OnEmit, scores each event against Scorecard rules,
// accumulates score. Result() returns pass/fail + breakdown.
//
// GOL-164, TSK-1088, TSK-1090
package referee
...
func (r *Referee) Result() Result {
	...
	if r.log != nil {
		...
		r.log.Log(context.Background(), level, "referee result",
			slog.String(telemetry.KeyAction, res.Name),
			slog.String(telemetry.KeyStatus, fmt.Sprintf("pass=%t score=%d/%d events=%d", res.Pass, res.Score, res.Threshold, len(res.Events))),
		)
	}
