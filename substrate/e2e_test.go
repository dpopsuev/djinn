...
func TestE2E_Skeleton(t *testing.T) {
	...
	scenario := crucible.NewStubScenario("http-service", "Build an HTTP server that returns JSON on GET /health")
	referee := crucible.NewStubReferee(crucible.CheckResult{Pass: true, Score: 0.9})

	// 3. Create a mock operator with canned prompts
	operator := crucible.NewMockOperator(
	...
	// 5. Record metrics
	metrics := crucible.RunMetrics{
		ScenarioID:  scenario.ID(),
		ToolLevel:   crucible.L0Mothballed,
		TimeElapsed: elapsed,
		TokensIn:    0,
		TokensOut:   0,
		Pass:        check.Pass,
