...
type MountEntry struct {
	...
	ScopeType   scope.ScopeType // scope semantics (system, operations, global, etc.)
}
...
func (t *MountTable) Mount(virtual, host string, readOnly bool, scopeType scope.ScopeType) error {
	virtual = cleanPath(virtual)
	host = cleanPath(host)
...
func (t *MountTable) Resolve(hostPath string) (string, error) {
	...
	for i := range t.entries {
		...
		if isUnder(hostPath, hp) && len(hp) > bestLen {
			e := t.entries[i]
			best = &e
			bestLen = len(hp)
		}
...
func (t *MountTable) longestMatch(virtualPath string) (MountEntry, bool) {
	...
	bestLen := 0
	for _, e := range t.entries {
		if isUnder(virtualPath, e.VirtualPath) && len(e.VirtualPath) > bestLen {
			best = e
...
// isUnder returns true if path is equal to or nested under root.
...
// cleanPath normalizes a path: cleans it and ensures consistent format.
