...
func TestVFS_PathEscape_Symlink(t *testing.T) {
	...
	mt := newTestTable()
	if err := mt.Mount("/workspace", hostRoot, false, ScopeSystem); err != nil {
		t.Fatal(err)
	}
	...
	if !isMountUnder(resolved, hostRoot) {
...
func TestVFS_MountInjection_ConflictRejected(t *testing.T) {
	...
	err := mt.Mount("/workspace/djinn", "/tmp/evil", false, ScopeSystem)
...
func TestVFS_MountInjection_NestedMountAllowed(t *testing.T) {
	...
	// Parent mount.
	if err := mt.Mount("/workspace", "/home/user/workspace", true, ScopeGlobal); err != nil {
		t.Fatal(err)
	}
