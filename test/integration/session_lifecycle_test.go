...
func TestSession_SaveAndLoad(t *testing.T) {
	...
	sess := cortex.New("lifecycle-test", "claude-opus", "/workspace")
	sess.Name = "lifecycle-test"
	sess.Append(cortex.Entry{Role: "user", Content: "hello"})
	sess.Append(cortex.Entry{Role: "assistant", Content: "hi"})
...
func TestSession_ResumeRestoresDriverModel(t *testing.T) {
	...
	sess := cortex.New("resume-test", "claude-opus-4-6", "/workspace")
...
func TestSession_ResumeRestoresWorkDirs(t *testing.T) {
	...
	sess := cortex.New("workdir-test", "model", "/workspace")
...
func TestSession_KillDeletes(t *testing.T) {
	...
	sess := cortex.New("kill-test", "model", "/workspace")
	sess.Name = "kill-test"
	store.Save(sess)
