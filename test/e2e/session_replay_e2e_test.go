...
func TestSession_SaveLoadReplay_ToolCallRoundTrip(t *testing.T) {
	...
	sess := cortex.New("e2e-replay", "claude-sonnet-4-6", "/workspace")
	sess.Name = "e2e-replay"
	sess.Driver = "claude"

	// Turn 1: user prompt
	sess.Append(cortex.Entry{Role: "user", Content: "list files in this directory"})

	// Turn 2: assistant calls a tool
	sess.Append(cortex.Entry{
		Role: "assistant",
	...
	sess.Append(cortex.Entry{Role: "assistant", Content: "I see three files: main.go, go.mod, and README.md."})

	// Turn 5: user follow-up
	sess.Append(cortex.Entry{Role: "user", Content: "read main.go"})
	...
	sess.Append(cortex.Entry{Role: "assistant", Content: "Here's main.go — it has a simple main function."})
...
func TestSession_SaveLoadReplay_CorruptToolUse(t *testing.T) {
	...
	sess := cortex.New("corrupt-e2e", "claude-sonnet-4-6", "/workspace")
	sess.Name = "corrupt-e2e"

	sess.Append(cortex.Entry{Role: "user", Content: "hello"})
	sess.Append(cortex.Entry{
		Role: "assistant",
		Blocks: []driver.ContentBlock{
			driver.NewTextBlock("Let me check."),
	...
	sess.Append(cortex.Entry{Role: "user", Content: "what happened?"})
