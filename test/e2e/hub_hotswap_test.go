...
func hubConnect(t *testing.T, socketPath, role string) *hotswap.SocketTransport {
	t.Helper()
	conn, err := net.Dial("unix", socketPath)
	if err != nil {
		t.Fatalf("dial hub as %s: %v", role, err)
	}
	if err := json.NewEncoder(conn).Encode(hotswap.RegisterMsg{Role: role}); err != nil {
		conn.Close()
		t.Fatalf("register as %s: %v", role, err)
	}
	return hotswap.WrapConn(conn)
}
...
func TestHub_E2E_ShellHotSwap(t *testing.T) {
	...
	defer cancel()
	defer h.Close()
	go h.Run(ctx) //nolint:errcheck // test helper, error checked elsewhere
	time.Sleep(100 * time.Millisecond)

	// Backend connects and stays alive the whole time.
	backend := hubConnect(t, sock, "backend")
	defer backend.Close()
	backend.SendToShell(hotswap.BackendMsg{Type: hotswap.BackendReady, Model: "test"}) //nolint:errcheck // best-effort send, error logged by receiver

	// Shell 1 connects, receives BackendReady.
	shell1 := hubConnect(t, sock, "shell")
	ready, err := shell1.RecvFromBackend()
	if err != nil || ready.Type != hotswap.BackendReady {
		t.Fatalf("shell1 ready: %+v, err: %v", ready, err)
	}

	// Shell 1 sends a prompt, backend receives it.
	shell1.SendToBackend(hotswap.ShellMsg{Type: hotswap.ShellPrompt, Text: "hello"}) //nolint:errcheck // best-effort send, error logged by receiver
	...
	// Backend responds.
	backend.SendToShell(hotswap.BackendMsg{Type: hotswap.BackendText, Text: "world"}) //nolint:errcheck // best-effort send, error logged by receiver
	...
	backend.SendToShell(hotswap.BackendMsg{Type: hotswap.BackendText, Text: "queued-after-crash"}) //nolint:errcheck // best-effort send, error logged by receiver
	time.Sleep(100 * time.Millisecond)
	...
	shell2.SendToBackend(hotswap.ShellMsg{Type: hotswap.ShellPrompt, Text: "from-rebuilt-shell"}) //nolint:errcheck // best-effort send, error logged by receiver
...
func TestHub_E2E_BackendHotSwap(t *testing.T) {
	...
	be1.SendToShell(hotswap.BackendMsg{Type: hotswap.BackendReady, Model: "v1"}) //nolint:errcheck // best-effort send, error logged by receiver
	...
	// Exchange messages.
	shell.SendToBackend(hotswap.ShellMsg{Type: hotswap.ShellPrompt, Text: "p1"}) //nolint:errcheck // best-effort send, error logged by receiver
	m1, _ := be1.RecvFromShell()
	if m1.Text != "p1" {
		t.Fatalf("be1 got %q", m1.Text)
	}
	be1.SendToShell(hotswap.BackendMsg{Type: hotswap.BackendText, Text: "r1"}) //nolint:errcheck // best-effort send, error logged by receiver
	...
	shell.SendToBackend(hotswap.ShellMsg{Type: hotswap.ShellPrompt, Text: "queued-prompt"}) //nolint:errcheck // best-effort send, error logged by receiver
	time.Sleep(100 * time.Millisecond)
	...
	be2.SendToShell(hotswap.BackendMsg{Type: hotswap.BackendReady, Model: "v2"}) //nolint:errcheck // best-effort send, error logged by receiver
...
func TestHub_E2E_FullRebuild(t *testing.T) {
	...
	be1.SendToShell(hotswap.BackendMsg{Type: hotswap.BackendReady, HistoryLen: 10}) //nolint:errcheck // best-effort send, error logged by receiver
	...
	// Backend 2 announces with restored cortex.
	...
	shell2.SendToBackend(hotswap.ShellMsg{Type: hotswap.ShellPrompt, Text: "alive"}) //nolint:errcheck // best-effort send, error logged by receiver
