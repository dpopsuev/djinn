...
func executeModel(cmd Command, sess *contextmgr.Session) CommandResult {
	if len(cmd.Args) > 0 {
...
func executeMode(cmd Command, sess *contextmgr.Session) CommandResult {
...
func executeStatus(sess *contextmgr.Session) CommandResult {
	return CommandResult{
...
func executeCost(sess *contextmgr.Session) CommandResult {
	return CommandResult{
		Output: fmt.Sprintf("tokens used: ~%d (approximate)", sess.TotalTokens()),
	}
}

func executeCompact(sess *contextmgr.Session) CommandResult {
	before, after := contextmgr.Compact(sess, contextmgr.DefaultKeepRecent)
	if before == after {
...
func executeMemory(sess *contextmgr.Session) CommandResult {
	name := sess.Name
	if name == "" {
		name = sess.ID
	}
...
func executeCopy(sess *contextmgr.Session) CommandResult {
...
func executePermissions(sess *contextmgr.Session) CommandResult {
	mode := sess.Mode
	if mode == "" {
...
func executeSessions(cmd Command, sess *contextmgr.Session) CommandResult {
	store, err := contextmgr.NewStore(sess.WorkDir)
	if err != nil {
	...
	matches := contextmgr.Search(list, query)
