// keybind.go — keybinding interface for the REPL.
// Provides a registry of named commands mapped to key sequences.
package tui

// KeyCommand is a named action that can be triggered by a keybinding.
...
// BindingTable provides key-to-command lookup.
type BindingTable interface {
	Lookup(key string) (KeyCommand, bool)
	Bindings() []Binding
}

// DefaultBindingTable returns the built-in keybinding table.
func DefaultBindingTable() BindingTable {
	return &stubBindingTable{}
}

type stubBindingTable struct{}
...
var defaultKeyCommands = map[string]KeyCommand{
...
// KeyMode represents a vim-style editing mode.
type KeyMode string

const (
	KeyModeInsert KeyMode = "insert"
...
func NewModeTable() *ModeTable {
	t := &ModeTable{
		mode:     KeyModeInsert,
		commands: defaultKeyCommands,
		bindings: map[KeyMode][]Binding{
			KeyModeInsert: defaultBindings,
			KeyModeNormal: {
...
func (t *ModeTable) SetMode(m KeyMode) { t.mode = m }

// CurrentMode returns the active mode.
func (t *ModeTable) CurrentMode() KeyMode { return t.mode }

// Lookup finds a command for the given key in the current mode.
func (t *ModeTable) Lookup(key string) (KeyCommand, bool) {
...
func (t *stubBindingTable) Lookup(key string) (KeyCommand, bool) {
	for _, b := range defaultBindings {
		if b.Key == key {
			cmd, ok := defaultKeyCommands[b.Command]
			return cmd, ok
		}
	}
	return KeyCommand{}, false
...
func (t *stubBindingTable) Bindings() []Binding {
