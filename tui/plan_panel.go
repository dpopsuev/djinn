...
func (p *PlanPanel) View(width int) string {
	p.width = width
	if p.graph == nil {
		return DimStyle.Render("No plan loaded")
	}
	...
	default:
		return DimStyle.Render("Unknown view")
	}
}
...
func (p *PlanPanel) renderOverview() string {
	...
	if len(all) == 0 {
		return DimStyle.Render("Empty plan")
	}
	...
	b.WriteString("\n")
	b.WriteString(Hint("j/k navigate", "enter drill", "q back"))
...
func (p *PlanPanel) renderGoalDetail() string {
	...
	if err != nil {
		return DimStyle.Render("Goal not found: " + p.goalID)
	}
	...
	if len(children) > 0 {
		for i := range children {
			...
			fileStr := ""
			if files > 0 {
				fileStr = DimStyle.Render(fmt.Sprintf(" (%d files)", files))
			}
	...
	if len(a.DependsOn) > 0 {
		b.WriteString("\n  ▲ depends: ")
		b.WriteString(DimStyle.Render(strings.Join(a.DependsOn, ", ")))
		b.WriteByte('\n')
	}
	...
	if a.Content != "" {
		b.WriteString("\n")
		b.WriteString(DimStyle.Render("  " + truncate(a.Content, 200))) //nolint:mnd // content preview
		b.WriteByte('\n')
	}

	b.WriteString("\n")
	b.WriteString(Hint("j/k navigate", "enter drill", "q overview"))
...
func (p *PlanPanel) renderSegmentDetail() string {
	...
	if err != nil {
		return DimStyle.Render("Segment not found: " + p.segID)
	}
	...
	if len(a.Components.Files) > 0 || len(a.Components.Directories) > 0 {
		...
		msg := RenderPanelMsg{Type: RenderTypeTree, Title: "Components", Data: treeData}
		b.WriteString(RenderPanel(msg, p.width))
		b.WriteByte('\n')
	}
	...
	// Dependencies.
	if len(a.DependsOn) > 0 {
		b.WriteString("\n  ▲ depends: " + DimStyle.Render(strings.Join(a.DependsOn, ", ")) + "\n")
	}
	...
	b.WriteString("\n")
	b.WriteString(Hint("q back", ":claim", ":+ :- :~ annotate"))
...
// Compile-time checks.
var (
	_ Panel                = (*PlanPanel)(nil)
	_ FocusContextProvider = (*PlanPanel)(nil)
)
