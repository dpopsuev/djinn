...
func TestServerPanel_GreenView(t *testing.T) {
	s := NewServerPanel("s1", "scribe", tui.StatusGreen, "http://localhost:8080/", nil)
	view := s.View(80)
...
func TestServerPanel_OfflineView(t *testing.T) {
	s := NewServerPanel("s2", "locus", tui.StatusOffline, "", nil)
	view := s.View(80)
...
func TestServerPanel_Children(t *testing.T) {
	...
	s := NewServerPanel("s1", "scribe", tui.StatusGreen, "", tools)
	if len(s.Children()) != 2 {
