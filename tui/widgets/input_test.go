...
func TestInputPanel_SetValueViaMessage(t *testing.T) {
	p := NewInputPanel()
	p.Update(tui.InputSetValueMsg{Value: "hello"})
	if p.Value() != "hello" {
...
func TestInputPanel_FocusBlurViaMessage(t *testing.T) {
	p := NewInputPanel()
	p.Update(tui.InputBlurMsg{})
	if p.Focused() {
		t.Fatal("should be blurred")
	}
	p.Update(tui.InputFocusMsg{})
	if !p.Focused() {
...
func TestInputPanel_AddHistoryViaMessage(t *testing.T) {
	p := NewInputPanel()
	p.Update(tui.InputAddHistoryMsg{Value: "first"})
	p.Update(tui.InputAddHistoryMsg{Value: "second"})
	p.HistoryUp()
	if p.Value() != "second" {
...
func TestInputPanel_CompletionsViaMessage(t *testing.T) {
	p := NewInputPanel()
	p.Update(tui.InputSetCompletionsMsg{Names: []string{"/help", "/config", "/clear"}})
	p.SetValue("/he")
...
func TestInputPanel_TabComplete_SingleMatch_AutoExecute(t *testing.T) {
	p := NewInputPanel()
	p.Update(tui.InputSetCompletionsMsg{Names: []string{"/help", "/config"}})
	p.SetFocus(true)
	p.Update(tui.InputSetValueMsg{Value: "/hel"})
	handled, cmd := p.TabComplete()
	if !handled {
...
func TestInputPanel_TabComplete_MultiMatch_NoAutoExecute(t *testing.T) {
	p := NewInputPanel()
	p.Update(tui.InputSetCompletionsMsg{Names: []string{"/config", "/config-save", "/clear"}})
	p.SetFocus(true)
	p.Update(tui.InputSetValueMsg{Value: "/config"})
	_, cmd := p.TabComplete()
	if cmd != nil {
...
func TestInputPanel_ResizeViaMessage(t *testing.T) {
	p := NewInputPanel()
	p.Update(tui.ResizeMsg{Width: 120, Height: 5})
...
func TestInputPanel_PredictionFromHistory(t *testing.T) {
	p := NewInputPanel()
	p.Update(tui.InputAddHistoryMsg{Value: "hello world"})
	p.Update(tui.InputAddHistoryMsg{Value: "help me"})
	p.SetFocus(true)
	p.Update(tui.InputSetValueMsg{Value: "hel"})
...
func TestInputPanel_PredictionEmpty(t *testing.T) {
	p := NewInputPanel()
	p.Update(tui.InputAddHistoryMsg{Value: "hello"})
	p.SetFocus(true)
	p.Update(tui.InputSetValueMsg{Value: "xyz"})
