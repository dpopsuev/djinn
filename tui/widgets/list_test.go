...
func makeItems(names ...string) []core.Panel {
	items := make([]core.Panel, len(names))
	for i, n := range names {
...
func TestListPanel_View_ShowsCursor(t *testing.T) {
	...
	if !strings.Contains(view, tui.ActiveGlyphs.ListCursor) {
		t.Fatalf("view should show cursor glyph %q: %q", tui.ActiveGlyphs.ListCursor, view)
	}
}
