...
func NewDriftPanel() *DriftPanel {
	return &DriftPanel{
		BasePanel: core.NewBasePanel(panelIDDrift, 4),
	}
}
...
func (d *DriftPanel) View(width int) string {
	...
	sb.WriteString("\n")
	sb.WriteString(tui.DimStyle.Render(fmt.Sprintf("Drift: %d tasks to convergence", d.tasksLeft)))
	return sb.String()
}
...
func renderPillarBar(name string, score float64, label string, width int) string {
	...
	bar := style.Render(strings.Repeat("\u2588", filled)) +
		tui.DimStyle.Render(strings.Repeat("\u2591", empty))

	return prefix + bar + pctStr + tui.DimStyle.Render(labelStr)
}
