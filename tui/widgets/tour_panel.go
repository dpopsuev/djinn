...
func (p *TourPanel) Children() []core.Panel { return nil }
...
func (p *TourPanel) Update(msg tea.Msg) (core.Panel, tea.Cmd) {
...
func (p *TourPanel) View(width int) string {
	...
	if p.tour == nil || len(p.tour.Circuits) == 0 {
		return tui.DimStyle.Render("No circuits to display. Run :tour to generate.")
	}
	...
	for i, sv := range cv.Stops {
		...
		name := sv.Name
		if sv.Changed {
			name = tui.ToolSuccessStyle.Render("● ") + name
		} else if sv.PassThrough {
			name = tui.DimStyle.Render("○ " + name)
		}

		b.WriteString(prefix + name)
		if sv.Detail != "" {
			b.WriteString(" — " + tui.DimStyle.Render(sv.Detail))
		}
		if sv.CrossRef != "" {
			b.WriteString(" " + tui.DimStyle.Render("["+sv.CrossRef+"]"))
		}
		b.WriteByte('\n')
	}
