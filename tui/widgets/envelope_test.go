...
func TestEnvelope_ToolResultViaMessage(t *testing.T) {
	...
	e.Update(tui.ToolResultMsg{Name: "Read", Output: "file contents\nline 2", IsError: false})
	if !e.Collapsed() {
...
func TestEnvelope_ErrorResult(t *testing.T) {
	...
	e.Update(tui.ToolResultMsg{Name: "Bash", Output: "permission denied", IsError: true})
	view := e.View(80)
...
func TestEnvelope_ToggleAfterResult(t *testing.T) {
	...
	e.Update(tui.ToolResultMsg{Name: "Read", Output: "content"})
	if !e.Collapsed() {
