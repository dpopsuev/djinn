...
var _ core.Panel = (*CommandsPanel)(nil)

func NewCommandsPanel(commands []string) *CommandsPanel {
	return &CommandsPanel{
		BasePanel: core.NewBasePanel(panelIDCommands, 0),
		commands:  commands,
	}
}
...
func (p *CommandsPanel) Update(msg tea.Msg) (core.Panel, tea.Cmd) {
	switch msg := msg.(type) {
	case tui.CommandsShowMsg:
		p.visible = true
		p.matches = filterCommands(p.commands, msg.Filter)
		p.cursor = 0
	case tui.CommandsHideMsg:
		p.visible = false
		p.matches = nil
		p.cursor = 0
	case tea.KeyMsg:
...
func (p *CommandsPanel) View(width int) string {
	...
	for i, cmd := range p.matches {
		prefix := "  "
		if i == p.cursor {
			prefix = tui.DimStyle.Render("▸ ")
		}
		sb.WriteString(prefix + tui.DimStyle.Render(cmd))
