...
func TestOdometerLine_UpdateFromMsg(t *testing.T) {
	o := NewOdometerLine()
	o.Update(tui.OdometerUpdateMsg{
		RoundTrips: 247,
		Cost:       6.77,
		Tasks:      14,
		Relays:     3,
...
func TestOdometerLine_UpdateIgnoresOtherMsgs(t *testing.T) {
	o := NewOdometerLine()
	o.Update(tui.OdometerUpdateMsg{RoundTrips: 10, Cost: 1.50, Tasks: 5, Relays: 1})
	// Send an unrelated message — values should not change.
	o.Update(tui.DashboardUIStateMsg{State: "STREAMING"})
...
func TestOdometerLine_UpdateReplacesValues(t *testing.T) {
	o := NewOdometerLine()
	o.Update(tui.OdometerUpdateMsg{RoundTrips: 10, Cost: 1.0, Tasks: 5, Relays: 1})
	o.Update(tui.OdometerUpdateMsg{RoundTrips: 20, Cost: 2.0, Tasks: 10, Relays: 2})
