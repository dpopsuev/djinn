...
var _ core.Panel = (*TurnEnvelope)(nil)
...
func NewTurnEnvelope(turnIdx int, userInput string) *TurnEnvelope {
	return &TurnEnvelope{
		BasePanel: core.NewBasePanel(fmt.Sprintf("turn-%d", turnIdx), 0),
		turnIdx:   turnIdx,
		userInput: userInput,
	}
}
...
func (t *TurnEnvelope) Children() []core.Panel {
	panels := make([]core.Panel, len(t.tools))
...
func (t *TurnEnvelope) Update(msg tea.Msg) (core.Panel, tea.Cmd) {
	return t, nil
}
...
func (t *TurnEnvelope) collapsedView() string {
	...
	return tui.DimStyle.Render(fmt.Sprintf("Turn #%d: %s", t.turnIdx, summary))
}
...
func (t *TurnEnvelope) expandedView(width int) string {
	...
	sb.WriteString(tui.UserStyle.Render(tui.LabelUser) + t.userInput)
	...
	if t.thinking != "" {
		sb.WriteByte('\n')
		sb.WriteString(tui.DimStyle.Render(tui.SpinnerFrames[0] + " " + t.thinking))
	}
	...
	turnBorderStyle := design.ActiveStyles.TurnBorder
	return turnBorderStyle.
		Width(innerWidth).
		BorderTop(true).
		BorderBottom(true).
		BorderLeft(true).
