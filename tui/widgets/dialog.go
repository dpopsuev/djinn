...
var _ core.Panel = (*DialogPanel)(nil)
...
func NewDialogPanel(title, message string, actions ...DialogAction) *DialogPanel {
	return &DialogPanel{
		BasePanel: core.NewBasePanel("dialog", 0),
		title:     title,
		message:   message,
		actions:   actions,
	}
...
func (p *DialogPanel) Update(msg tea.Msg) (core.Panel, tea.Cmd) {
	if msg, ok := msg.(tea.KeyMsg); ok {
		...
		case tea.KeyEnter:
			return p, func() tea.Msg {
				return tui.DialogResultMsg{ActionID: p.SelectedAction()}
			}
		case tea.KeyEsc:
			return p, func() tea.Msg {
				return tui.DialogResultMsg{ActionID: "cancel"}
			}
		}
	}
	return p, nil
